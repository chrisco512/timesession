...
const { EVT_ADJ_CLOCKED_OUT } = require('../events/names');
const Events = require('../events');
const state = require('../state');

function* adjClockOut() {
	//try to build event with payload
	let eventResult;
	
	//throw 400 if can't create new time session
	try {
		eventResult = yield Events[EVT_ADJ_CLOCKED_OUT](state.timeSessions, this.request.body);
	} catch(err) {
